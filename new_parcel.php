            ...
            <?php if($login_branch_id <= 0): ?>
              ...
              <select name="from_branch_id" id="from_branch_id" class="form-control select2" required="">
                ...
                if($login_type == 1) { // Admin
                    ...
                    $branches = $conn->query("SELECT * FROM branches WHERE id = '$login_branch_id'");
                }
                while($row = $branches->fetch_assoc()):
                ?>
                <option value="<?php echo $row['id'] ?>" <?php echo $login_branch_id == $row['id'] ? 'selected' : '' ?>><?php echo $row['branch_code'] . ' | ' . $row['city'] ?></option>
                <?php endwhile; ?>
              </select>
            </div>
            <?php else: ?>
              <input type="hidden" name="from_branch_id" value="<?php echo $login_branch_id ?>">
            <?php endif; ?>  
