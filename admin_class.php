...
Class Action {
	...
	function login(){
		...
		// Get user details with branch info
		$qry = $this->db->query("SELECT u.*, b.branch_code, b.city,
		...
		if($qry->num_rows > 0){
			...
			// Set branch_id for staff users
			if($row['type'] == 2) {
				if(empty($row['branch_id'])) {
					// Try to get branch assignment
					$branch_check = $this->db->query("SELECT branch_id FROM users WHERE id = '".$row['id']."'");
	...
	function get_parcel_heistory(){
		...
		// Sanitize input
		$ref_no = $this->db->real_escape_string($ref_no);
		
		// Get parcel details
		$parcel = $this->db->query("SELECT p.*, 
			...
			WHERE p.reference_number = '$ref_no'");
		...
		$history = $this->db->query("SELECT pt.*, 
			b1.branch_code as from_branch,
			b2.branch_code as to_branch 
			FROM parcel_tracks pt 
			LEFT JOIN branches b1 ON pt.from_branch_id = b1.id 
			LEFT JOIN branches b2 ON pt.to_branch_id = b2.id 
			WHERE pt.parcel_id = {$parcel['id']} 
			ORDER BY pt.date_created ASC");
